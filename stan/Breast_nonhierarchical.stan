...
model {  
  ...
  for (i in 1:N_uc) {
    t_uc[i] ~ weibull(r, exp(-(alpha + beta_age * age_uc[i] + beta_npi * npi_uc[i] +  beta_horm * horm_uc[i] + beta_claudin * claudin_uc[i]) / r));
  } 
  for (i in 1:N_rc) {
    
    1 ~ bernoulli(exp(-pow(t_rc[i] / exp(-(alpha + beta_age * age_rc[i] + beta_npi * npi_rc[i] +  + beta_horm * horm_rc[i] + beta_claudin * claudin_rc[i]) / r), r)));
  }
}
